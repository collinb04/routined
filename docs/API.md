...
| POST | `/api/problems/:id/struggle/tutor-chat` | session | `{ goal: "explore" \| "identify" \| "approach", messages: [...], problemContext?, options?, targetInsight? }` | The **ungated** tutor backing the Explore/Identify/Approach tabs — no prior commit required. Returns `{ content }`. |
