export default {
  ...
  clues: [
    {
      id: 'directed-reach',
      question: 'Bomb A detonates bomb B if B\'s center is within A\'s radius, but not necessarily the other way around. What kind of graph does this produce?',
      options: [
    ...
    {
      id: 'constraint-size',
      question: 'bombs.length ≤ 100. What complexity is acceptable for this problem?',
      options: [
    ...
    {
      id: 'edge-condition',
      question: 'Bomb A reaches bomb B when dist(A, B) ≤ r_A. You compute dist(A, B) = sqrt((x_A−x_B)² + (y_A−y_B)²). What implementation detail avoids a pitfall?',
      options: [
    ...
    },
    {
      ...
      question: 'Detonating bomb A can trigger bomb B, which then triggers bomb C. What graph algorithm captures this chain?',
      options: [
