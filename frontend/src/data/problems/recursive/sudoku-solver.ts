export default {
  ...
  starterCode: `class Solution:
    def solve_sudoku(self, board):
        pass`,
  functionName: 'solve_sudoku_run',
  conceptId: 'backtracking',
  runnerSetup: `def solve_sudoku_run(board):
  ...
  bruteHint: 'The brute-force approach tries every digit 1-9 in every empty cell without checking validity along the way, then only verifies the board once it\'s completely filled. With 81 cells and up to 9 choices each, that is as many as 9^81 combinations to blindly generate before finding one that is fully valid — computationally infeasible. It never eliminates a bad branch early, so enormous work is wasted filling cells that were doomed from the start. What if you validated each digit the moment you placed it, instead of waiting until the whole board is done?',
  optimizeComplexity: { time: 'O(9^(empty cells))', space: 'O(1)' },
  clues: [
    {
      id: 'constraint-fixed-board-size',
      highlight: { location: 'constraint', text: 'board.length == board[0].length == 9' },
      question: 'Constraint bounds tell you what time complexity is actually feasible before you write a single line of code. The board is always 9×9 — a fixed size, not parameterized. What does this tell you about complexity analysis?',
      options: [
    ...
    },
    {
      ...
      highlight: { location: 'constraint', text: 'The solution is unique' },
      question: 'Guarantees stated in the problem can let you skip work that would otherwise be necessary for correctness. "The solution is unique." What does this let you do during backtracking?',
      options: [
        ...
        { label: 'Try digits in any order without undoing wrong placements', isCorrect: false, feedback: 'Uniqueness means exactly one solution exists, not that any sequence of choices leads there. You still need backtracking to undo wrong placements.' },
    ...
    },
    {
      ...
      question: 'Understanding every rule that defines a valid state is essential before you can check that state efficiently. Sudoku validity requires each digit 1–9 to appear once per row, once per column, and once per 3×3 box. What does this imply about your placement check?',
      options: [
        ...
        { label: 'Check constraints only after undoing a failed placement', isCorrect: false, feedback: 'Checking only at backtrack time means you place invalid digits first and undo them after — the opposite of what you want. Check before placing to prune invalid branches early.' },
      ],
    ...
    },
    {
      ...
      question: 'A function\'s signature is itself a contract — it tells you what the caller expects back, not just what algorithm to run. The function signature is solve_sudoku(board) with no return value. What does this tell you about how to deliver the answer?',
      options: [
  ...
  solutionCode: `class Solution:
    def solve_sudoku(self, board):
        ...
        backtrack()`,
  solutionComplexity: { time: 'O(9^(empty cells))', space: 'O(1)' },
  solutionCaveat: 'The inner loop <code>returns False</code> the moment every digit 1-9 has been tried at a cell with no success — that failure propagates straight back up to the previous placement, which is what triggers undoing it (<code>board[r][c] = \'.\'</code>) and trying its next candidate digit, rather than the search getting stuck.',
  solutionExplanation: 'Checking a candidate digit against its row, column, and 3×3 box before placing it prunes invalid branches immediately, rather than filling the whole board first and discovering the conflict only at the end — that early validity check is what keeps the exponential search space manageable in practice. Because the puzzle is guaranteed to have exactly one solution, the recursive <code>backtrack()</code> can return <code>True</code> and unwind immediately the moment every cell is filled, with no need to keep searching for alternative completions once the first one is found — and since <code>board</code> is mutated directly rather than copied, the caller simply reads the modified board after the function returns.',
}
