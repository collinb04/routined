  ...
  clues: [
    {
      ...
      question: 'S(i, j) = prefix[j+1] − prefix[i]. The condition lower ≤ S(i,j) ≤ upper becomes lower ≤ prefix[j+1] − prefix[i] ≤ upper. What problem does this reduce to?',
      options: [
        ...
        { label: 'Sorting the array to group sums by value', isCorrect: false, feedback: 'Sorting the original array destroys contiguity — subarrays must be consecutive elements, so element order matters.' },
      ],
    ...
    },
    {
      ...
      question: 'nums.length ≤ 10⁵. A brute-force O(n²) scan of all subarrays would be…',
      options: [
    ...
    },
    {
      ...
      question: 'For each new prefix sum, you need to count how many previous prefix sums fall in [prefix[j] − upper, prefix[j] − lower]. What structure supports this efficiently?',
      options: [
        { label: 'A hash map of exact prefix-sum values', isCorrect: false, feedback: 'A hash map handles exact-value lookups in O(1), but a range query [lo, hi] requires iterating all keys — O(n) per query, back to O(n²) total.' },
        { label: 'A sorted list with binary-search range counting', isCorrect: true },
        { label: 'A stack of all previous prefix sums', isCorrect: false, feedback: 'A stack only gives O(1) access to the top element — a range count requires searching the entire stack, which is O(n) per query.' },
        { label: 'A two-pointer on the original array', isCorrect: false, feedback: 'Two pointers work when the condition is monotone (e.g., a fixed target sum), but range-sum counting with arbitrary bounds is not monotone over prefix sums.' },
      ],
    ...
    {
      id: 'output-is-count',
      question: 'The output is an integer count of qualifying ranges. What does this mean for your algorithm?',
      options: [
