export default {
  ...
  starterCode: `class Solution:
    def three_sum_closest(self, nums, target):
        pass`,
  runnerSetup: 'three_sum_closest = Solution().three_sum_closest',
  ...
  bruteHint: 'The brute-force approach checks every possible triplet of indices — three nested loops trying every combination of i, j, k and computing how close each triplet\'s sum is to target, updating a running best whenever a closer sum turns up. That works, but it checks on the order of n³ triplets. At n up to 500, that\'s roughly 125 million operations — would that finish in time?',
  optimizeComplexity: { time: 'O(n²)', space: 'O(log n)' },
  clues: [
    {
      id: 'constraint-complexity',
      question: 'We can understand how efficient we need to be based on the size constraint of the input. What does 3 ≤ nums.length ≤ 500 tell you?',
      options: [
      ...
      highlight: { location: 'constraint', text: '3 ≤ nums.length ≤ 500' },
    },
    {
      id: 'output-type',
      question: 'The type of output you\'re asked for tells you how much of the problem you actually need to solve. The output is the closest sum, not the triplet itself — what does this change?',
      options: [
    ...
    },
    {
      ...
      question: 'The technique you reach for often only works once one property of the input already holds. To use two pointers after fixing one element, what must be true first?',
      options: [
    ...
    },
    {
      ...
      question: 'Specific wording about what you\'re optimizing for can tell you when you\'ve already found the best possible answer. If the current triplet sum equals target exactly, what can you do?',
      options: [
      ...
      highlight: { location: 'description', text: 'the sum is closest to' },
    },
  ],
  ...
  solutionExplanation: 'Sorting first turns the search for two more elements into a two-pointer scan: for a fixed <code>nums[i]</code>, moving the left pointer right only ever increases the sum and moving the right pointer left only ever decreases it, so every step is a deliberate move toward <code>target</code> rather than a guess. Fixing each of the n elements as <code>nums[i]</code> and running an O(n) two-pointer scan for the other two gives O(n²) overall, and the running <code>closest</code> variable just keeps the best sum seen across every fixed choice of <code>i</code>.',
}
