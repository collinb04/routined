  ...
  clues: [
    {
      ...
      question: 'A collision only happens when a positive asteroid is immediately followed by a negative one. What does this tell you about processing order?',
      options: [
        ...
        { label: 'Two-pointer scan from both ends', isCorrect: false, feedback: 'Collisions only happen rightward — a left-moving asteroid hits the nearest surviving asteroid to its left, not whatever is at the far right end.' },
      ],
    ...
    {
      id: 'output-structure',
      question: 'The output is the surviving asteroids in their original order. This means you need to…',
      options: [
        { label: 'Collect survivors into a set', isCorrect: false, feedback: 'A set loses order and cannot store duplicates — two asteroids of the same size can coexist if they move the same direction.' },
    ...
    {
      id: 'equal-size-rule',
      question: '"Equal-sized asteroids both explode." What does this add to your collision logic?',
      options: [
    ...
    },
    {
      ...
      question: 'asteroids.length ≤ 10⁴ and each asteroid has absolute value ≤ 1000. What complexity is acceptable?',
      options: [
