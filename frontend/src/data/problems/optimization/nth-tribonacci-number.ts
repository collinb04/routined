export default {
  ...
  starterCode: `class Solution:
    def tribonacci(self, n):
        pass`,
  runnerSetup: 'tribonacci = Solution().tribonacci',
  ...
  bruteHint: 'The brute-force approach follows the recurrence directly: compute tribonacci(n) by recursively calling tribonacci(n-1), tribonacci(n-2), and tribonacci(n-3) with no caching of results. Because each call branches into three more calls, the same subproblems get recomputed many times over, giving roughly O(3ⁿ) time. Even at n = 37, that is hundreds of billions of redundant calls. When computing tribonacci(n-1), does that call not already recompute tribonacci(n-2) and tribonacci(n-3) internally, the exact values you are about to compute again from scratch?',
  optimizeComplexity: { time: 'O(n)', space: 'O(1)' },
  clues: [
    {
      id: 'constraint-small-n',
      question: 'Constraints often tell you what time complexity is acceptable before you write any code. n ≤ 37 tells you…',
      highlight: { location: 'constraint', text: '0 ≤ n ≤ 37' },
      options: [
    ...
    },
    {
      ...
      question: 'A recurrence relation tells you exactly how many prior values each new computation depends on, and therefore what state you must keep around. Tₙ₊₃ = Tₙ + Tₙ₊₁ + Tₙ₊₂. This means each value depends on…',
      highlight: { location: 'description', text: 'Tₙ₊₃ = Tₙ + Tₙ₊₁ + Tₙ₊₂' },
      options: [
    ...
    {
      id: 'base-cases',
      question: 'Every recurrence needs base cases to bottom out on, values that cannot be derived from the formula itself. T₀=0, T₁=1, T₂=1 are explicitly given. This means…',
      highlight: { location: 'description', text: 'T₀=0, T₁=1, T₂=1' },
      options: [
  ...
  solutionCaveat: 'All three base cases (<code>T₀=0</code>, <code>T₁=1</code>, <code>T₂=1</code>) are seeded before the loop even considers running — the three-term recurrence has no way to derive any of them from earlier terms, since there simply are no terms before <code>T₀</code>.',
  solutionExplanation: 'Each Tribonacci number depends on exactly the three before it, so three rolling variables carrying the most recent values forward is all the recurrence ever needs — every iteration shifts <code>(a, b, c)</code> to <code>(b, c, a+b+c)</code>, discarding the oldest value since it will never be referenced again. The three explicit base cases (n=0, 1, 2) are handled before the loop starts because the loop\'s very first iteration already assumes all three seed values exist.',
}
