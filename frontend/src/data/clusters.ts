...
export const CLUSTERS: Cluster[] = [
  ...
  },
  {
    ...
    concepts: [
      ...
      { id: 'recursion-dp-bridge', label: 'Recursion to DP Bridge', bridge: true },
    ],
    transferHooks: [
  ...
  },
  {
    ...
    why: `2D traversal, grid BFS/DFS, matrix rotation, and multi-pass patterns all share the same ground truth: a grid is an array of arrays, addressed by (r, c) instead of a single index, with up to four neighbors instead of two. Traversal patterns are just different orders for sweeping that address space — row by row, diagonally, or in a shrinking spiral. Grid BFS/DFS recognizes that a grid is a graph in disguise: each cell is a node, and its in-bounds neighbors are its edges, so every graph traversal technique transfers directly. Matrix rotation decomposes into simpler moves you already know — transpose plus reverse. Multi-pass patterns show up whenever a single sweep can't see enough context to update a cell correctly, and the fix is splitting the work into passes with a narrower job each. None of this is a new data structure — it's the array you already understand, addressed in two dimensions.`,
    concepts: [
      { id: 'grid-intro', label: 'Intro to Grid & Matrix' },
      { id: '2d-array-traversal', label: '2D Array Traversal' },
      { id: 'grid-bfs-dfs', label: 'Grid BFS & DFS' },
      { id: 'matrix-rotation', label: 'Matrix Rotation & Transformation' },
